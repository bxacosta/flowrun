...
const captureFlow = browser.flow({
    name: "capture",
    nodes: ({ task }) => [
        task({
            ...
            run: async (context) => {
                ...
                const result = await context.storage.save(
                    `${PREFIX}home.png`,
                    new Uint8Array(screenshot),
                    { page: "home", source: "screenshot" }
                );
        ...
        task({
            ...
            run: async (context) => {
                ...
                for (const key of page.keys) {
                    ...
                    context.log.info(
                        `  ${key}  size=${info.size}  modified=${info.modifiedAt.toISOString()}`
                    );
                }
                if (page.nextCursor) {
